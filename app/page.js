...
const SK = 'ps-src-v3', CK = 'ps-cache-v3', TTL = 20 * 60 * 1000;
  ...
  return (
    ...
    >
      ...
      <div style={{ padding: 'clamp(14px, 3vw, 20px)', display: 'flex', flexDirection: 'column', gap: 6, flex: 1 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11, color: '#999', letterSpacing: 0.2 }}>
          ...
          {article.pubDate && (
            <>
              ...
              <span>{new Date(article.pubDate).toLocaleTimeString('de-DE', { hour: '2-digit', minute: '2-digit' })}</span>
            </>
          )}
        </div>
        ...
        {article.summary && (
          <p style={{
            margin: 0, fontSize: 'clamp(13px, 2vw, 14px)',
    ...
    <div style={{ minHeight: '100dvh', background: '#faf9f7' }}>
      ...
      <div style={{
        ...
        <button onClick={onBack} style={{
          ...
          cursor: 'pointer', fontSize: 13, fontFamily: "'Source Sans 3', sans-serif",
          padding: 0, fontWeight: 600, letterSpacing: 0.3,
        }}>
          <span style={{ marginRight: 6 }}>&larr;</span> ZURUCK
        </button>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <a href={article.url} target="_blank" rel="noopener noreferrer" title="Originalseite" style={{ color: '#999', display: 'flex', alignItems: 'center', marginLeft: 4 }}>
            <IconExternal />
          </a>
        </div>
      </div>
      ...
      {heroImage && (
        <div style={{
          width: '100%', height: 'clamp(200px, 35vw, 360px)',
      ...
      <div style={{ maxWidth: 640, margin: '0 auto', padding: 'clamp(20px, 5vw, 40px) clamp(16px, 4vw, 32px)' }}>
        {article.pubDate && (
          <div style={{ fontSize: 12, color: '#999', marginBottom: 10, textTransform: 'uppercase', letterSpacing: 0.4, fontVariantNumeric: 'tabular-nums' }}>
            {new Date(article.pubDate).toLocaleDateString('de-DE', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}
          </div>
        )}

        <h1 style={{ fontSize: 'clamp(24px, 5vw, 36px)', fontWeight: 700, lineHeight: 1.18, fontFamily: "'Libre Baskerville', serif", color: '#111', marginBottom: 16, letterSpacing: '-0.3px' }}>
          {article.title}
        </h1>

        {article.summary && (
          <p style={{ fontSize: 'clamp(15px, 2.3vw, 18px)', lineHeight: 1.6, color: '#666', fontStyle: 'italic', marginBottom: 28 }}>
            {article.summary}
          </p>
        )}
          ...
          <div style={{ padding: '32px 0', textAlign: 'center' }}>
            {error === 'paywall' ? (
              <div style={{ color: '#999', fontSize: 14 }}>
                <div style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 8, color: '#c0392b', fontWeight: 600 }}>Bezahlinhalt</div>
                Dieser Artikel erfordert ein Abonnement.
              </div>
            ) : (
            ...
            <a href={article.url} target="_blank" rel="noopener noreferrer" style={{
              ...
              border: '1px solid #ddd', color: '#555', textDecoration: 'none', fontSize: 13,
            }}>Originalseite <IconExternal /></a>
          </div>
        )}

        {content && (
          <div style={{ fontSize: 'clamp(15px, 2.3vw, 17px)', lineHeight: 1.85, color: '#333', whiteSpace: 'pre-wrap', fontFamily: "'Libre Baskerville', serif" }}>
            {content}
          </div>
        )}
      </div>
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999, padding: 20 }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#faf9f7', borderRadius: 16, padding: 'clamp(24px, 5vw, 36px)', width: '100%', maxWidth: 400, boxShadow: '0 20px 60px rgba(0,0,0,0.15)' }}>
        <h2 style={{ margin: '0 0 24px', fontSize: 18, fontFamily: "'Libre Baskerville', serif", fontWeight: 700 }}>Quelle hinzufugen</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{ display: 'flex', gap: 12, marginTop: 12 }}>
            ...
            <button onClick={submit} style={{ flex: 1, padding: '13px 0', borderRadius: 8, border: 'none', background: '#111', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer', opacity: url.trim() ? 1 : 0.3 }}>Hinzufugen</button>
          </div>
        </div>
      </div>
    </div>
    ...
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.3)', backdropFilter: 'blur(6px)', zIndex: 9998, display: 'flex', justifyContent: 'flex-end' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: '#faf9f7', width: 'min(320px, 85vw)', height: '100%', padding: '32px 24px', boxShadow: '-8px 0 40px rgba(0,0,0,0.1)', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          <h3 style={{ fontSize: 16, fontWeight: 700, fontFamily: "'Libre Baskerville', serif" }}>Quellen verwalten</h3>
...
export default function Home() {
  ...
  const remove = (id) => { setSources((p) => p.filter((s) => s.id !== id)); setArticles((p) => { const n = { ...p }; delete n[id]; return n; }); setErrors((p) => { const n = { ...p }; delete n[id]; return n; }); if (activeFilter === id) setActiveFilter('all'); };
  const refresh = () => { setArticles({}); setErrors({}); load([...sources]); };

  const openArticle = (article) => {
    if (feedRef.current) setScrollY(feedRef.current.scrollTop);
    setReader(article);
  };
  const closeReader = () => {
    setReader(null);
    requestAnimationFrame(() => {
      if (feedRef.current) feedRef.current.scrollTop = scrollY;
    });
  };
  ...
  if (reader) return (
    <>
      ...
      <Reader article={reader} onBack={closeReader} />
    </>
  );
  ...
  return (
    <>
      ...
      <div ref={feedRef} style={{ height: '100dvh', overflowY: 'auto', background: '#f5f4f0', fontFamily: "'Source Sans 3', sans-serif" }}>
        {/* Header */}
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: sources.length > 0 ? 10 : 0 }}>
            ...
            <div style={{ display: 'flex', gap: 8 }}>
              {sources.length > 0 && (
                <button onClick={refresh} disabled={anyLoad} style={{ background: 'none', border: 'none', color: anyLoad ? '#ccc' : '#888', fontSize: 15, cursor: anyLoad ? 'not-allowed' : 'pointer', padding: '4px 8px' }}>
                  <svg width="14" height="14" viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round">
                    <path d="M1 2v5h5" /><path d="M15 14v-5h-5" />
                    <path d="M2.5 10.5A6 6 0 0114 8" /><path d="M13.5 5.5A6 6 0 012 8" />
                  </svg>
                </button>
              )}
          ...
          {sources.length > 0 && (
            <div style={{ display: 'flex', gap: 6, overflowX: 'auto', WebkitOverflowScrolling: 'touch', paddingBottom: 2 }}>
              ...
              {sources.map((s) => (
                <button key={s.id} onClick={() => setActiveFilter(s.id)} style={{
                  padding: '5px 12px', borderRadius: 100, fontSize: 12,
        ...
        {/* Content */}
        <div style={{ padding: 'clamp(12px, 3vw, 24px) clamp(12px, 3vw, 24px)', maxWidth: 900, margin: '0 auto' }}>
          {/* Errors */}
          {errs.length > 0 && (
            <div style={{ marginBottom: 12, padding: '10px 14px', borderRadius: 10, background: '#fef2f2', border: '1px solid #fecaca', fontSize: 12, color: '#991b1b' }}>
              {errs.map(([id, msg]) => {
                const s = sources.find((x) => x.id === id);
                return <div key={id} style={{ marginTop: 2 }}><strong>{s?.name}:</strong> {msg}</div>;
              })}
            </div>
          )}
          ...
          {sources.length === 0 && (
            <div style={{ textAlign: 'center', padding: 'clamp(60px, 15vw, 120px) 20px', color: '#999' }}>
              ...
              <h2 style={{ fontFamily: "'Libre Baskerville', serif", fontSize: 'clamp(20px, 4vw, 28px)', fontWeight: 700, textAlign: 'center', lineHeight: 1.3, color: '#333', marginBottom: 12 }}>
                Dein Nachrichtenmorgen
              </h2>
              <p style={{ fontSize: 14, color: '#888', maxWidth: 340, margin: '0 auto 28px', lineHeight: 1.6 }}>
                Fuege Nachrichtenquellen hinzu. 100+ RSS-Feeds werden automatisch erkannt.
              </p>
              <button onClick={() => setModal(true)} style={{ padding: '13px 28px', borderRadius: 8, border: 'none', background: '#111', color: '#fff', fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Erste Quelle hinzufugen</button>
            </div>
          )}
          ...
          {allArticles.length > 0 && (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(min(100%, 360px), 1fr))', gap: 'clamp(10px, 2vw, 16px)' }}>
              {allArticles.map((a) => (
                <FeedCard key={a.id} article={a} onClick={() => openArticle(a)} />
              ))}
            </div>
          )}

          {allArticles.length > 0 && (
            <div style={{ textAlign: 'center', padding: '32px 0', color: '#ccc', fontSize: 12 }}>
              {allArticles.length} Artikel
            </div>
          )}
        </div>
      </div>
